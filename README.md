...
- NPC 离场后隐藏但保留历史，再次登场时恢复关系、心理轨迹和日记。
- 数据按聊天写入 `chat_metadata`，不会加入或回注主聊天 Prompt。
- 圆形入口按钮支持鼠标与触屏拖动并记忆位置；入口与玻璃拟态悬浮窗绑定移动，移动端 Bottom Sheet 可下滑关闭。
...
- 小剧场状态存放在当前聊天的元数据键 `npc_theater_v1`。
...
- 拖动圆形 🎭 入口或悬浮窗标题栏时，两者会作为一组同步移动并记忆位置；轻点入口仍会打开小剧场。
- 手机云端部署同时监听触摸松开与普通点击，避免浏览器不生成 click 时无法打开。
- 触屏设备即使报告桌面宽度也会进入 Bottom Sheet；打开后短暂忽略幽灵遮罩点击，避免面板在顶部闪现后立即关闭。
- 手机端面板无位移动画直接出现在最终位置，并在打开后的 800ms 内屏蔽所有可能被重新定向到关闭按钮的延迟点击。
- 移动端使用 Bottom Sheet 时面板固定，向下滑动面板关闭；圆形入口仍可独立移动。
